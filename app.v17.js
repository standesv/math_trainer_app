...
function showScreen(name) {
  ...
  // HARD routing: use inline display styles, not [hidden]
  Object.entries(screens).forEach(([k, el]) => {
    if (!el) { try { showToast(`Écran manquant: ${k}`); } catch {} ; return; }
    el.style.display = (k === name) ? "flex" : "none";
    el.hidden = false; // avoid UA quirks
  });
  currentScreen = name;
  try {
...
// ---------------- Storage ----------------
const PROFILES_KEY = "math_trainer_profiles_v17";
const ACTIVE_PROFILE_KEY = "math_trainer_active_profile_v17";
...
function historyKeyFor(profile) { return `math_trainer_history_v17::${profile}`; }
function progressKeyFor(profile) { return `math_trainer_progress_v17::${profile}`; }
...
// V16: initialize display states deterministically
try {
  ensureScreens();
  Object.entries(screens).forEach(([k, el]) => { if (el) el.style.display = (k === "home") ? "flex" : "none"; });
} catch {}
